...
num_sim_train=50 # number of simulations of each class of training sets, note that the training sets should be balanced
num_sim_test=50 # number of simulations of each class of testing sets, note that the testing sets should be balanced
...
sh run_RAiSD-AI.sh -i $1 -a SweepNet -o $2 -w $RAiSD_AI_win -l $length -t $target -e $epochs -d $d_type -n $3 -b $mbs

# Run FAST-NN

sh run_RAiSD-AI.sh -i $1 -a FAST-NN -o $2 -w $RAiSD_AI_win -l $length -t $target -e $epochs -d $d_type -n $3 -b $mbs

# Run FASTER-NN

sh run_RAiSD-AI.sh -i $1 -a FASTER-NN -o $2 -w $RAiSD_AI_win -l $length -t $target -e $epochs -d $d_type -n $3 -b $mbs

# Run FASTER-NN-G8

sh run_RAiSD-AI.sh -i $1 -a FASTER-NN-G -o $2 -w $RAiSD_AI_win -l $length -t $target -e $epochs -d $d_type -g $group -n $3 -b $mbs

# Run FASTER-NN-G128

sh run_RAiSD-AI.sh -i $1 -a FASTER-NN-G -o $2 -w $RAiSD_AI_win -l $length -t $target -e $epochs -d $d_type -g 128 -n $3 -b $mbs
