...
show_help() {
    ...
    echo
    echo -e "sh run_T-REx.sh training_neutral_ms_file_path training_selective_sweep_ms_file_path testing_neutral_ms_file_path testing_selective_sweep_ms_file_path outout_folder_path window_size length number_of_training_simulations number_of_testing_simulations"
    echo
...
./convert -i $1 -m original -c bp -w $6 -s 5000 -l $7 -r 5 -g 1 -o "$5"extracted_data/neutral_train.ms
./convert -i $2 -m original -c bp -w $6 -s 5000 -l $7 -r 5 -g 1 -o "$5"extracted_data/selsweep_train.ms
./convert -i $3 -m original -c bp -w $6 -s 5000 -l $7 -r 5 -g 1 -o "$5"extracted_data/neutral_test.ms
...
python3 TOOLS/T-REx/train_ms.py $8 1 "$5"MS_files_train/
python3 TOOLS/T-REx/train_ms.py $8 0 "$5"MS_files_train/
python3 TOOLS/T-REx/parse_train.py $8 1 "$5"MS_files_train/ "$5"CVF_files/
python3 TOOLS/T-REx/parse_train.py $8 0 "$5"MS_files_train/ "$5"CVF_files/
...
start=$(date +"%Y-%m-%d %H:%M:%S");
python3 TOOLS/T-REx/test_ms.py $9 1 "$5"MS_files_test/
python3 TOOLS/T-REx/test_ms.py $9 0 "$5"MS_files_test/
python3 TOOLS/T-REx/parse_test.py $9 1 "$5"MS_files_test/ "$5"CVF_files/
python3 TOOLS/T-REx/parse_test.py $9 0 "$5"MS_files_test/ "$5"CVF_files/
end=$(date +"%Y-%m-%d %H:%M:%S");
start_s=$(date -d "$start" +%s);
end_s=$(date -d "$end" +%s);
echo "$((end_s-start_s)) " >> "$5"Results/Results.txt
