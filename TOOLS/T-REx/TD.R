...
{
  i
  mean_sweep <- read.csv(paste0(input,"sweep_test_align_",i,".csv"), header = TRUE, row.names = 1)
  dim(mean_sweep)
  #mean_sweep<-as.matrix(mean_sweep)
...
{
  i
  mean_sweep <- read.csv(paste0(input,"neut_test_align_",i,".csv"), header = TRUE, row.names = 1)
  dim(mean_sweep)
  #mean_sweep<-as.matrix(mean_sweep)
  addd<-abind(addd, mean_sweep, along = 3)
  print(i)
